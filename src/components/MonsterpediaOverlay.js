...
export const MonsterpediaOverlay = ({
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="w-full bg-[#383a37] text-[#8fa07e] text-[12px] px-2 py-1.5 flex justify-between items-center mb-2 font-black shadow-md border-b-2 border-[#1a1a1a]">
                <span>像素怪獸圖鑑</span>
                <span className="text-[10px] opacity-80">{ownedMonsters.length} / {monsterIds.length}</span>
            </div>

            {/* Grid */}
            <div className="flex-1 w-full grid grid-cols-4 gap-1.5 overflow-hidden content-start px-1 py-1 bg-[#91a37c] rounded-sm border border-[#383a37]">
                {pageItems.map((id, idx) => {
                    ...
                    return (
                        <div 
                            key={id}
                            className={`relative aspect-square border-2 flex items-center justify-center rounded-sm transition-all overflow-hidden
                                ${isSelected ? 'border-[#ff5252] z-10 shadow-lg scale-110' : 'border-[#383a37] shadow-inner'}
                                ${owned ? 'bg-[#f0f4e4]' : 'bg-[#383a37]'}`}
                        >
                            <DitheredSprite 
                                id={id} 
                                scale={0.8} 
            ...
            <div className="w-full mt-2 bg-[#869675] border border-[#383a37] p-1.5 rounded flex items-center justify-between shadow-md">
                <div className="flex flex-col">
                    <span className="text-[10px] font-black text-[#1a1a1a] opacity-60">
                ...
                <div className="text-[9px] font-black text-[#383a37] flex flex-col items-end opacity-80">
                    <span>[A] 切換 [B] 詳細</span>
                    <span>[C] 返回</span>
                </div>
            </div>

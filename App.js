...
export default function App() {
    ...
    const loginWithGoogle = async () => {
        if (!auth || !googleProvider) {
            ...
            setAlertMsg("系統尚未啟動: Firebase 初始化失敗。請確認網路連線或稍後再試。");
            return;
        }
        ...
        try {
            ...
            if (result.user) {
                ...
                updateDialogue("⚠️ 登入已取消或找不到使用者資訊。", true);
            }
        } catch (e) {
            console.error("☁️ Login Error:", e);
            let errMsg = e.message;
            if (e.code === 'auth/popup-closed-by-user') errMsg = "登入視窗被關閉了。";
            if (e.code === 'auth/unauthorized-domain') errMsg = "此網域尚未被授權，請檢查 Firebase 設定。";
            if (e.code === 'auth/operation-not-allowed') errMsg = "請在 Firebase 控制台啟用 Google 登入方法。";
    ...
    const handleB = (clickIdx = null) => {
        ...
        if (isPvpMode && matchStatus === 'matched' && battleState.active && battleState.phase === 'player_action') {
            const currentIdx = battleState.menuIdx || 0;
            const move = battleState.player?.moves?.[currentIdx];
            if (move) {
                executeBattleTurn('attack', move);
            } else {
                const tempLogs = [...battleState.logs, "尚未裝備技能！"];
        ...
        if (battleState.active && battleState.mode === 'trainer' && battleState.phase === 'player_action') {
            const currentIdx = battleState.menuIdx || 0;
            const move = battleState.player?.moves?.[currentIdx];
            if (move) {
                executeBattleTurn('attack', move);
                ...
                // 如果空發
                const tempLogs = [...battleState.logs, "該格子尚未裝備技能！"];
                setBattleState(prev => ({ ...prev, logs: tempLogs.slice(-5) }));
                playBloop('fail');
            }
            return;
